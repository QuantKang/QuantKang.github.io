...
[^fn_tech_metric_c_sma]: <small>C-SMA(i, j, k, ...)는 주가가 이동평균선 i, j, k, ...를 모두 상회하거나 반대로 모두 하회하는 경우 매수하여 보유하는 전략입니다.</small>
[^fn_tecn_metric_macd]: <small> MACD(i, j, k)는 단기 지수 이동평균선 i, 장기 지수 이동평균선 j, MACD 시그널 지수 이동평균선 k를 사용하는 전략입니다. MACD가 시그널보다 위에 있으면 매수하고, 반대로 아래에 있으면 매도합니다.</small>
