...
[^fn_lev_vn_exepnse]: <small>변동성 비용은 연속적이지 않은 기초 상품의 일단위 수익률을 레버리지 배율만큼 추종하는 과정에서 발생하는 변동성 끌림에 의한 발생하는 비용입니다.</small>
[^fn_lev_fn_exepnse]: <small>금융 비용은 계획된 레버리지 배율을 맞추기 위해 기초 자산을 보유한 타기관에 스왑(Swap) 등의 형태로 차입하는데 필요한 비용합니다. 예를 들어 투자자 자금만큼 은행에서 대출 받아 2배 레버리지를 한다면, 은행에서 빌린 자금에 대한 대출이자가 계속 나가게 됩니다. 이 비용이 금융 비용입니다.</small>
[^fn_lev_tt_exepnse]: <small>변동성 비용과 금융 비용을 합한 것입니다. 이 비용은 산술합으로 계산되지 않습니다. 예를 들어 변동성 비용이 10%, 금융 비용이 10%라면 전체 비용은 20%가 아닌 1 - (1 - 10%) * (1 - 10%) = 19%입니다.</small>
